...
CREATE TABLE `doctors` (
  ...
  `gender` ENUM('MALE', 'FEMALE', 'OTHER') NULL COMMENT 'Biological gender',
  `date_of_birth` DATE NULL COMMENT 'Doctor date of birth',
  ...
  `address` VARCHAR(255) NULL COMMENT 'Residential/office address',
  `profile_photo` VARCHAR(255) NULL COMMENT 'Profile photo placeholder or URL',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `patients` (
  ...
  `allergies` TEXT NULL COMMENT 'Known allergies description',
  `medical_conditions` TEXT NULL COMMENT 'Pre-existing medical conditions',
  `current_medications` TEXT NULL COMMENT 'Current active medications',
  `insurance_provider` VARCHAR(100) NULL COMMENT 'Health insurance company name',
  `insurance_policy_number` VARCHAR(100) NULL COMMENT 'Insurance policy or ID number',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `appointments` (
  ...
  `status` ENUM('PENDING', 'SCHEDULED', 'CONFIRMED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'NO_SHOW') NOT NULL DEFAULT 'PENDING' COMMENT 'Appointment status lifecycle',
  `type` ENUM('FIRST_VISIT', 'FOLLOW_UP', 'ROUTINE_CHECKUP', 'EMERGENCY') NOT NULL DEFAULT 'FIRST_VISIT' COMMENT 'Visit classification',
  `appointment_mode` ENUM('ONLINE', 'OFFLINE') NOT NULL DEFAULT 'OFFLINE' COMMENT 'Consultation mode (Online / Offline)',
  `reason` VARCHAR(255) NULL COMMENT 'Reason for consultation',
  `symptoms` TEXT NULL COMMENT 'Detailed patient reported symptoms',
  `consultation_notes` TEXT NULL COMMENT 'Doctor notes and diagnosis',
...
CREATE TABLE `bills` (
  ...
  `patient_id` BIGINT UNSIGNED NOT NULL COMMENT 'Foreign key to patients table',
  `doctor_id` BIGINT UNSIGNED NULL COMMENT 'Foreign key to doctors table',
  `department_id` BIGINT UNSIGNED NULL COMMENT 'Foreign key to departments table',
  `appointment_id` BIGINT UNSIGNED NULL COMMENT 'Associated appointment if applicable',
  `consultation_fee` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Consultation charges',
  `lab_charges` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Lab diagnostic test charges',
  `medicine_charges` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Pharmacy medication charges',
  `procedure_charges` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Surgical or medical procedure fees',
  `room_charges` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Bed or room stay charges',
  `additional_charges` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Miscellaneous charges',
  `total_amount` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Subtotal of item charges',
  ...
  `grand_total` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Grand total payable amount',
  ...
  `payment_status` ENUM('Pending', 'Paid', 'Partially Paid', 'Failed', 'Refunded', 'Cancelled', 'UNPAID', 'PARTIALLY_PAID', 'PAID') NOT NULL DEFAULT 'Pending' COMMENT 'Payment state',
  `payment_method` VARCHAR(50) NULL COMMENT 'Payment mode e.g. Cash, UPI, Credit Card, Online Payment Gateway',
  `transaction_id` VARCHAR(100) NULL COMMENT 'Transaction reference ID',
  `razorpay_order_id` VARCHAR(100) NULL COMMENT 'Razorpay Gateway Order ID',
  `razorpay_payment_id` VARCHAR(100) NULL COMMENT 'Razorpay Gateway Payment ID',
  `razorpay_signature` VARCHAR(255) NULL COMMENT 'Razorpay HMAC Signature',
  `due_date` DATE NULL COMMENT 'Payment deadline date',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT `fk_bills_patients` FOREIGN KEY (`patient_id`) REFERENCES `patients` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `fk_bills_doctors` FOREIGN KEY (`doctor_id`) REFERENCES `doctors` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `fk_bills_departments` FOREIGN KEY (`department_id`) REFERENCES `departments` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  ...
  INDEX `idx_bills_doctor_id` (`doctor_id`),
